...
function casper_setup() {
	...
	register_nav_menus( array(
		'primary' => esc_html__( 'Primary', 'casper' ),
	) );
 ...
 */
function casper_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'casper_content_width', 640 );
}
add_action( 'after_setup_theme', 'casper_content_width', 0 );
...
function casper_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'casper' ),
		'id'            => 'sidebar-1',
		'description'   => '',
...
function casper_scripts() {
	...
	wp_enqueue_script( 'casper-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'casper-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
...
function add_query_vars($aVars) {
	$aVars[] = "param"; // represents the name of the product category as shown in the URL
	return $aVars;
}
